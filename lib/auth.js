...
const defaultOptions = { allow: false, checkUser: false };
...
export const withUserAuth = (handler, options = defaultOptions) => {
	return async (req, res) => {
		...
		// TODO: remove this condition before prod
			...
			if (!basic_usage_limit_email) {
				try {
					await resend.sendEmail({
						...
						to: user.email,
						react: (
							<UsageExceededEmail limit={basicPlanUsageLimit} maxUsageLimit={premiumPlanUsageLimit} plan="Basic Plan" />
						),
					});
